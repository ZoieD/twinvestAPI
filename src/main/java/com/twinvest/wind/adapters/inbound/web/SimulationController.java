...
public class SimulationController {
    ...
    // 关键改动：用 @Qualifier 指定要注入的 bean 名称
    // 这两个名字需要与 BeanConfig 中 @Bean 方法名一致：
    //   - runSimulationUseCase()
    //   - getSimulationUseCase()
    public SimulationController(
            @Qualifier("runSimulationUseCase") RunSimulationUseCase runUC,
            @Qualifier("getSimulationUseCase") GetSimulationUseCase getUC
    ) {
        this.runUC = runUC;
        this.getUC = getUC;
    }
    ...
    public SimulationDto run(@RequestBody RunRequestDto dto) {
        Scenario sc = new Scenario(
                dto.nTurbines(), dto.pRatedKw(), dto.dtHours(),
                dto.windSpeed(), dto.priceEurMwh()
        );
        SimulationResult r = runUC.run(dto.assetId(), sc);
        return new SimulationDto(
                r.getId(), r.getAssetId(), r.getTime(), r.getPowerKw(),
